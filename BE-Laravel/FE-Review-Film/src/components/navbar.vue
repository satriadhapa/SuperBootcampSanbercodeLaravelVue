  ...
  <nav class="bg-gradient-to-r from-gray-500 to-indigo-400 p-4 shadow-lg">
    <div class="container mx-auto flex justify-between items-center">
      ...
      <div class="hidden md:flex space-x-4 items-center">
        ...
        <router-link to="/profile" v-if="isLoggedIn" class="text-white text-lg hover:underline">Profile</router-link>
        <router-link to="/update-user" v-if="isLoggedIn" class="text-white text-lg hover:underline">Update User</router-link>
        <router-link to="/login" v-if="!isLoggedIn" class="text-white text-lg hover:underline">Login</router-link>
        <router-link to="/register" v-if="!isLoggedIn" class="text-white text-lg hover:underline">Register</router-link>
        <router-link to="/verify-account" v-if="isRegistering" class="text-white text-lg hover:underline">Verify Account</router-link>
        <button @click="logout" v-if="isLoggedIn" class="text-white text-lg hover:underline">Logout</button>
      </div>
      <div class="md:hidden">
    ...
    <div :class="{'block': isOpen, 'hidden': !isOpen}" class="md:hidden">
      ...
      <router-link to="/profile" v-if="isLoggedIn" class="block text-white text-lg hover:underline px-4 py-2">Profile</router-link>
      <router-link to="/update-user" v-if="isLoggedIn" class="block text-white text-lg hover:underline px-4 py-2">Update User</router-link>
      <router-link to="/login" v-if="!isLoggedIn" class="block text-white text-lg hover:underline px-4 py-2">Login</router-link>
      <router-link to="/register" v-if="!isLoggedIn" class="block text-white text-lg hover:underline px-4 py-2">Register</router-link>
      ...
      <button @click="logout" v-if="isLoggedIn" class="block text-white text-lg hover:underline px-4 py-2">Logout</button>
    </div>
  </nav>
</template>
